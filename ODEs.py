...
class BackwardEuler(ConstantTimestep):
    ...
    '''
    def __init__(self, dfun, xzero, timerange, timestep,
                 convergencethreshold=1, maxiterations=30,args=None):
        ...
        self.origin_fun = dfun
        if args is not None:
            # Wrap the user's fun (and jac, if given) in lambdas to hide the
            # additional parameters.  Pass in the original fun as a keyword
            # argument to keep it in the scope of the lambda.
            dfun = lambda t, x, fun=dfun: fun(t, x, *args)
            print(dfun)
        ...
        if self.stepcounter < self.steps:
            if self.status == 'initialized':
                ...
                while (
                    ...
                    xn1 = xn2
                    # print("iterations:%d,error:%10.20f"%(iterations, error))
                if error <= self.convergencethreshold:
                    self.time, self.x = (self.timestart + (self.stepcounter * self.timestep),xn1)
                    return self.time, self.x
                else:
                    return self.time, self.x
    ...
    def step(self, args=None):
        if args is not None:
            ...
            # additional parameters.  Pass in the original fun as a keyword
            # argument to keep it in the scope of the lambda.
            self.dfun = lambda t, x, fun=self.origin_fun: fun(t, x, *args)
        t_column, X = next(self)
        return t_column, X

def backwardeuler(dfun, xzero, timerange, timestep, args=None):
